...
本專案成功實現了一套基於電腦視覺的手勢辨識物聯網控制系統，涵蓋了從資料收集、模型訓練、即時推論到硬體控制的完整流程。系統能夠即時辨識「張開」與「握拳」兩種手勢，並無線控制 ESP32 上的 LED 燈號。

## 8.2 未來展望

- **SQL 資料庫整合**：記錄每次手勢觸發的時間與次數，提供統計分析
- **多手勢擴充**：增加更多手勢類別（如比數字、指向等）
- **網頁儀表板**：開發前端介面即時監控系統狀態與歷史紀錄
- **邊緣運算優化**：將模型量化並部署至 ESP32 內建運算單元，降低對 PC 的依賴
- **多裝置協作**：支援同時控制多組 ESP32 裝置
