...
def is_path_safe(abs_path):
    ...
    ws = os.path.normpath(os.path.expandvars(WORKSPACE))
    return os.path.commonpath([abs_path, ws]) == ws

AUTO_AUTHORIZE = False
TEST_DENY_EXTERNAL = False

def authorize(message):
    if AUTO_AUTHORIZE:
        return True
    if TEST_DENY_EXTERNAL and "外部" in message:
        return False
    print(f"\n⚠️  {message}")
...
def handle_read_file(path):
    safe = is_path_safe(resolve_path(path))
...
def extract_referenced_paths(command):
    """Extract likely file path references from a shell command."""
    ...
    for token in tokens:
        ...
        if not token or token.startswith('-'):
            continue
        if any(c in token for c in ('/', '\\')) or \
           os.path.isabs(token) or \
           token.startswith('..') or \
           (token.startswith('.') and len(token) > 1 and token[1] in ('/', '\\')) or \
           re.match(r'^[a-zA-Z]:[\\/]', token):
            paths.add(token)

    return list(paths)

def check_shell_safety(command):
    """Check if a shell command accesses files outside the workspace.
    Returns list of (original_path, resolved_path) for unsafe accesses.
    """
    unsafe = []
    for path in extract_referenced_paths(command):
        try:
            resolved = resolve_path(path)
            if not is_path_safe(resolved):
                unsafe.append((path, resolved))
        except Exception:
            pass
    return unsafe
...
def load_model():
    global _model, _tokenizer, _generate
    if _model is None:
        ...
        _generate = generate
        print("模型載入完成！")
    return _model, _tokenizer, _generate
        ...
        while True:
            ...
            for _, tool_type, arg1, arg2 in tools:
                ...
                if tool_type == 'shell':
                    ...
                    unsafe_paths = check_shell_safety(cmd)
                    if unsafe_paths:
                        path_info = "; ".join(f"'{orig}'" for orig, _ in unsafe_paths)
                        if not authorize(f"LLM 請求透過 shell 存取外部檔案：{path_info}"):
